    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
                .flatMapToPair( kv -> {
                    ...
                    Website links = kv._2._1();
                    Double currentRank = kv._2._2();
                    Integer numDests = links.getNEdges();
                    Double contrib = currentRank / numDests;

                    List<Tuple2 <Integer, Double>> contributions = new LinkedList<>();

                    Iterator<Integer> iterLinks = links.edgeIterator();
                    while(iterLinks.hasNext()) {
                        Integer destId = iterLinks.next();
                        Tuple2<Integer, Double> t = new Tuple2<>(destId, contrib);
                        contributions.add(t);
                    }
        ...
        return newRanks.reduceByKey( (c1, c2) -> c1 + c2).mapValues(
                v -> 0.15 + (0.85 * v));
