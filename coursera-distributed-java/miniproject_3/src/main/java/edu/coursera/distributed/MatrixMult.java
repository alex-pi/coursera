...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int localRank = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD);
        final int numRanks = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD);
        final int nrows = c.getNRows();
        final int chunkSize = (nrows + numRanks -1 ) / numRanks;
        final int startRow = localRank * chunkSize;
        int endRow = (localRank+1) * chunkSize;
        if(endRow > nrows) endRow = nrows;

        mpi.MPI_Bcast(a.getValues(), 0, a.getNCols() * a.getNRows(), 0,
                mpi.MPI_COMM_WORLD);
        mpi.MPI_Bcast(b.getValues(), 0, b.getNCols() * b.getNRows(), 0,
                mpi.MPI_COMM_WORLD);
        ...
        if(localRank == 0) {
            MPI.MPI_Request[] reqs = new MPI.MPI_Request[numRanks-1];
            for(int i = 1 ; i < numRanks; i++) {
                ...
                reqs[i-1] = mpi.MPI_Irecv(c.getValues(), rowOffset, nElements, i, i, mpi.MPI_COMM_WORLD);
            }
            mpi.MPI_Waitall(reqs);
        } else {
            mpi.MPI_Send(c.getValues(), startRow * c.getNCols(),
                    (endRow - startRow) * c.getNCols(), 0, localRank, mpi.MPI_COMM_WORLD);
        }
    }
}
