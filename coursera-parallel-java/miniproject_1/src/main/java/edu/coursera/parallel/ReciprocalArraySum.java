...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if(endIndexExclusive - startIndexInclusive <= THRESHOLD) {
                //System.out.println("Sequential calculation for : (" + startIndexInclusive + "," + endIndexExclusive + "]" );
                for (int i = this.startIndexInclusive; i < this.endIndexExclusive; i++) {
                    this.value += 1 / input[i];
                }
            } else {
    ...
    protected static double parArraySum(final double[] input) {
        ...
        ReciprocalArraySumTask left = new ReciprocalArraySumTask(0, half, input);
        left.fork();
        ReciprocalArraySumTask right = new ReciprocalArraySumTask(half, input.length, input);
        right.compute();
        left.join();
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        for (int chunk=0; chunk < numTasks; chunk ++ ) {
            int start = getChunkStartInclusive(chunk, numTasks, input.length);
            int end = getChunkEndExclusive(chunk, numTasks, input.length);
