...
public class ReciprocalArraySumTest extends TestCase {
    ...
    public void testParSimpleOneMillion() {
        ...
        System.out.println("testParSimpleOneMillion speedup = " + speedup);
    ...
    public void testParSimpleOneHundredMillion() {
        ...
        final String errMsg = String.format("It was expected that the two-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        System.out.println("testParSimpleOneHundredMillion speedup = " + speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    }
    ...
    public void testParManyTaskOneMillion() {
        ...
        final String errMsg = String.format("It was expected that the many-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        System.out.println("testParManyTaskOneMillion speedup = " + speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    }
    ...
    public void testParManyTaskOneHundredMillion() {
        final int ncores = getNCores();
        System.out.println("testParManyTaskOneHundredMillion ncores = " + ncores);
        final double speedup = parTestHelper(100_000_000, true, ncores);
        final double minimalExpectedSpeedup = (double)ncores * 0.8;
        final String errMsg = String.format("It was expected that the many-task parallel implementation would run at " +
                "least %fx faster, but it only achieved %fx speedup", minimalExpectedSpeedup, speedup);
        System.out.println("testParManyTaskOneHundredMillion speedup = " + speedup);
        assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
    }
}
